    ...
    if ($id > 0) {
        ...
        if ($transaction) {
            switch ($action) {
                ...
                case 'reject_post':
                    if (hasDepartment('Accounts') && $transaction['leasing_post_status'] === 'pending') {
                        $result = $transactionModel->rejectTransaction($id, 'leasing', $userId, $userName);
                        if ($result) {
                            $success_msg = "Transaction post rejected successfully.";
                        } else {
                            $error_msg = "Error rejecting transaction post. Please try again.";
                        }
                    }
                    break;
                ...
                case 'reject_account':
                    if (hasDepartment('Accounts') && $transaction['approval_status'] === 'pending') {
                        $result = $transactionModel->rejectTransaction($id, 'account', $userId, $userName);
                        if ($result) {
...
// Get transactions based on department
if (hasDepartment('Wealth Creation')) {
    // Wealth Creation officers only see their own transactions
    $db = new Database();
